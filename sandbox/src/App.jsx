...
class ClassCounter extends React.Component {
  ...
  render() {
    return <>
      <div>ClassCounter — {this.state.count}</div>
...
// Prediction:
//   ClassFollowButton  → alerts the CURRENT user at alert-fire time (@userB)
//                        because `this.props` is a live reference — it always
//                        reads whatever props the component has RIGHT NOW.
//   FunctionFollowButton → alerts the user at CLICK time (@userA)
//                          because `username` is closed over from the render
//                          that produced this particular click handler.

// TODO: implement ClassFollowButton (class component)
class ClassFollowButton extends React.Component {
  // Arrow class field ensures `this` is bound correctly.
  // NOTE: `this.props.username` is read INSIDE the setTimeout callback,
  // so it resolves at alert-fire time — not at click time.
  handleClick = () => {
  ...
  render() {
    return (
      <button onClick={this.handleClick}>
        Follow {this.props.username} (class)
      </button>
    );
  }
}
...
function FunctionFollowButton({ username }) {
  // `username` is captured from this render's closure.
  // Even if the parent re-renders with a new username, the closure
  // here still holds the value that existed when this handler was created.
  const handleClick = () => {
